...
npm:@xynogen/pix-voice|Provider-neutral push-to-talk dictation plus the transcribe and speak tools — each provider needs its own API key.
...
npm:@xynogen/pix-proc|proc tool — run and manage long-lived processes (npm run dev, vite, python) that outlive a turn; spawns background processes, so enable it explicitly.
"
