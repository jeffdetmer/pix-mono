 ...
 *
 * Steering only — these teach a leaner tool without blocking. Privileged auth
 * stand-ins (bash `sudo`/`ssh`) are NOT here: they are *guarded* (hard block +
 * redirect) by pix-gate, a separate concern. Guard = stop; nudge = steer.
 */
